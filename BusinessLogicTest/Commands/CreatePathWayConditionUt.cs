...
public class CreatePathWayConditionUt
{
    ...
    public void Execute_CreatesPathWayConditionBetweenGivenObjects()
    {
        ...
        var previousSpace = new Space("a", "b", "c", "d", "e", 5, null!,200,200);
        var previousPathWay = new Pathway(previousCondition, previousSpace);
        ...
        world.Spaces.Add(previousSpace);
        world.Pathways.Add(previousPathWay);
        ConditionEnum condition = ConditionEnum.And;
        bool actionWasInvoked = false;
        Action<World> mappingAction = _ => actionWasInvoked = true;
        ...
        Assert.That(world.Spaces.Count, Is.EqualTo(1));
        Assert.That(world.Pathways.Count, Is.EqualTo(1));
        Assert.IsFalse(actionWasInvoked);
        ...
        Assert.That(world.Spaces.Count, Is.EqualTo(1));
        Assert.That(world.Pathways.Count, Is.EqualTo(3));
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(world.SelectedObject, Is.EqualTo(pathWayConditionRedo));
        });
    }
    ...
    public void UndoRedo_UndoesAndRedoesCreateSpace()
    {
        ...
        Assert.That(world.PathWayConditions, Has.Count.EqualTo(2));
        Assert.That(world.SelectedObject, Is.EqualTo(command.PathWayCondition));
        Assert.IsTrue(actionWasInvoked); actionWasInvoked = false;
